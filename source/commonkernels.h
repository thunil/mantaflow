...
KERNEL(idx) InvertCheckFluid (FlagGrid& flags, Grid<Real>& grid)
{
...
KERNEL(idx, reduce) struct GridSumSqr (Grid<Real>& grid)
{
    double sum;
...
KERNEL(bnd=1) CurlOp (const Grid<Vec3>& grid, Grid<Vec3>& dst) {
...
KERNEL(bnd=1) DivergenceOpMAC(Grid<Real>& div, const MACGrid& grid) {
...
KERNEL(bnd=1) GradientOpMAC(MACGrid& gradient, const Grid<Real>& grid) {
    gradient(i,j,k) = (Vec3(grid(i,j,k)) - Vec3(grid(i-1,j,k), grid(i,j-1,k), grid(i,j,k-1)));
}

//! Kernel: gradient operator 
KERNEL(bnd=1) GradientOp(Grid<Vec3>& gradient, const Grid<Real>& grid) {
...
KERNEL(bnd=1) GetShiftedComponent(const Grid<Vec3>& grid, Grid<Real>& comp, int dim) {
    Vec3i ishift(i,j,k);
    ishift[dim]--;
...
KERNEL(idx) GetComponent(const Grid<Vec3>& grid, Grid<Real>& comp, int dim) {
    comp[idx] = grid[idx][dim];
};

//! Kernel: get norm of centered grid
KERNEL(idx) GridNorm(Grid<Real>& n, const Grid<Vec3>& grid) {
    n[idx] = norm(grid[idx]);
};

//! Kernel: set component (not shifted)
KERNEL(idx) SetComponent(Grid<Vec3>& grid, const Grid<Real>& comp, int dim) {
    grid[idx][dim] = comp[idx];
};

//! Kernel: compute centered velocity field from MAC
KERNEL(bnd=1) GetCentered(Grid<Vec3>& center, const MACGrid& vel) {
    center(i,j,k) = 0.5*(vel(i,j,k)+Vec3(vel(i+1,j,k).x, vel(i,j+1,k).y, vel(i,j,k+1).z));
};

//! Kernel: compute MAC from centered velocity field
KERNEL(bnd=1) GetMAC(MACGrid& vel, const Grid<Vec3>& center) {
