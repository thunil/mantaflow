...
#fileFormat      =     ph.getParam( "fileFormat",      fileFormat) # fixed
...
#model.add( keras.layers.convolutional.Conv2D(clFMs, (3,3), activation='relu', input_shape=(16,16,n_inputChannels) ) )
model.add( keras.layers.Conv2D(clFMs, (3,3), activation='relu', input_shape=(16,16,n_inputChannels), padding='same' ) )
model.add( keras.layers.MaxPooling2D(pool_size=(pool,pool)) )

model.add( keras.layers.Dense(4*4*clFMs, activation='relu') )
# NT_DEBUG model.add(Dropout(0.25)) test?

#model.add( TransposedConvolution2D(clFMs, 3,3, activation='relu') )
#model.add( keras.layers.Conv2DTranspose(clFMs, (3,3), activation='relu') )
model.add( keras.layers.convolutional.Conv2DTranspose(clFMs, (3,3), activation='relu', strides=(2,2), padding='same' ) )
model.add( keras.layers.convolutional.Conv2DTranspose(4, (3,3), activation='relu', strides=(2,2), padding='same' ) )
model.add( keras.layers.convolutional.Conv2DTranspose(2, (3,3), activation='relu', strides=(2,2), padding='same' ) )
model.add( keras.layers.convolutional.Conv2DTranspose(1, (5,5), activation='relu', strides=(2,2), padding='same' ) )
...
#model.fit( tiCr.tile_data['inputs_train'], tiCr.tile_data['outputs_train'], batch_size=batchSize, epochs=int(trainingEpochs/batchSize) )
if not outputOnly:
	if 1:
		lastCost = 1e10
		lastSave = 1
		saveInterval = 5 # NT_DEBUG
		kerasChunk   = 100
		save_no = 0
		...
		for epoch in range(trainingEpochs):
			...
			model.fit( batch_xs, batch_ys, batch_size=batchSize, epochs=1 )
	...
	tdataSize = tiCr.tile_data['inputs_test'].shape[0]
	tiCr.tile_data['inputs_test']  = np.reshape( tiCr.tile_data['inputs_test'],  [tdataSize,tileSizeLow,tileSizeLow,  n_inputChannels] )
	tiCr.tile_data['outputs_test'] = np.reshape( tiCr.tile_data['outputs_test'], [tdataSize,tileSizeHigh,tileSizeHigh,1 ] )
	resultTiles = model.predict( tiCr.tile_data['inputs_test'] )
